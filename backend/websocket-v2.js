...
handleProtocol = async (obj, ws, w) => {
    ...
    if (methods.includes(obj.type)) {
        ...
        if (obj.type === "device_card_read") {
            ...
            console.log(time);
            console.log(date);
            console.log(new Date(2000 + date[2]*1, date[0]*1 - 1, date[1], time[0], time[1], time[2]));
            let attendance = new Attendance;
            attendance.chip = obj['chip'];
            attendance.emp_id = obj['emp_id'];
            attendance.time = new Date(2000 + date[2]*1, date[0]*1 - 1, date[1], time[0], time[1], time[2]);
            attendance.save();
